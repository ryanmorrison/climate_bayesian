...
future_150 <- climate.hydrology("data/HAD_future_150.csv")
...
future_150_stage <- stage(future_150, head_conv)
future_150 <- data.frame(future_150, future_150_stage)

#### Calculate recession rates ####
# The 1-, 2-, ... , 7-day, 14-day recession rates were computed
recess_1d <- recession.rate.forward(future_150, 1)
recess_2d <- recession.rate.forward(future_150, 2)
recess_3d <- recession.rate.forward(future_150, 3)
recess_4d <- recession.rate.forward(future_150, 4)
recess_5d <- recession.rate.forward(future_150, 5)
recess_6d <- recession.rate.forward(future_150, 6)
recess_7d <- recession.rate.forward(future_150, 7)
recess_14d <- recession.rate.forward(future_150, 14)
recess_30d <- recession.rate.forward(future_150, 30)
recess_45d <- recession.rate.forward(future_150, 45)

future_150 <- cbind(future_150, recess_1d, recess_2d, recess_3d, recess_4d, recess_5d, recess_6d, recess_7d, recess_14d, recess_30d, recess_45d)
colnames(future_150)[5:14] <- c("recess_1d", "recess_2d", "recess_3d", "recess_4d", "recess_5d", "recess_6d", "recess_7d", "recess_14d", "recess_30d", "recess_45d")
...
q_bin_prob <- q.prob(future_150, q_bin)
...
future_150_monthly <- subset.month(future_150)

apr_may <- list.to.df(future_150_monthly[[1]])
jun_jul <- list.to.df(future_150_monthly[[2]])
aug_sep <- list.to.df(future_150_monthly[[3]])
...
timing_state <- timing.states(future_150)

# Discrete states (Y, N) based on inundation
q1_state <- q.states(future_150, q_bin, 1)
q2_state <- q.states(future_150, q_bin, 2)
q3_state <- q.states(future_150, q_bin, 3)
q4_state <- q.states(future_150, q_bin, 4)
q5_state <- q.states(future_150, q_bin, 5)
q6_state <- q.states(future_150, q_bin, 6)
q7_state <- q.states(future_150, q_bin, 7)

# Discrete states based on recession rates
recess_state <- recess.states(future_150, recess_rates, "recess_14d")